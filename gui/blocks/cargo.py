...
Two-grid layout for correct column alignment without blocking move/resize:

  Header Gtk.Grid (in body, outside scroll) — static, 3 rows:
    row 0: blank | blank | StationName | Gal. Avg
    row 1: Item  | Qty.  | Sell        | blank
    row 2: separator (colspan 4)

  Data Gtk.Grid (inside ScrolledWindow) — dynamic rows appended/removed:
    category rows, item rows, totals rows

Both grids use identical column widths:
  col 0: hexpand  col 1: _W_QTY+_M_QTY  col 2: _W_SELL  col 3: _W_AVG

Since both grids are the same pixel width and use the same fixed column
widths, columns align pixel-perfectly across the header/data boundary.
"""
...
_W_SELL = 82
_W_AVG  = 82
_M_QTY  = 8   # margin_end on qty column
...
def _attach_row(grid, row, col0, col1_text, col2_text, col3_text, col0_css="data-key"):
    """Attach a 4-column row to a grid at the given row index."""
    grid.attach(col0,                                                 0, row, 1, 1)
    grid.attach(_lbl(col1_text, 1.0, "data-key", width=_W_QTY,  margin_end=_M_QTY), 1, row, 1, 1)
    grid.attach(_lbl(col2_text, 1.0, "data-key", width=_W_SELL),                     2, row, 1, 1)
    grid.attach(_lbl(col3_text, 1.0, "data-key", width=_W_AVG),                      3, row, 1, 1)
...
class CargoBlock(BlockWidget):
    ...
    def build(self, parent: Gtk.Box) -> None:
        ...
        # ── Header grid (static, outside scroll) ─────────────────────────────
        self._hdr_grid = Gtk.Grid()
        self._hdr_grid.set_column_spacing(4)
        self._hdr_grid.set_margin_start(4)
        self._hdr_grid.set_margin_end(12)
        self._hdr_grid.set_hexpand(True)
        self._hdr_grid.set_vexpand(False)
        ...
        self._hdr_grid.attach(_r0c0,              0, 0, 1, 1)
        self._hdr_grid.attach(_r0c1,              1, 0, 1, 1)
        self._hdr_grid.attach(self._mkt_loc_lbl,  2, 0, 1, 1)
        self._hdr_grid.attach(_r0c3,              3, 0, 1, 1)

        # Row 1: Item | Qty. | Sell | blank
        _r1c0 = _lbl("Item",  css="data-key", hexpand=True)
        _r1c1 = _lbl("Qty.",  xalign=1.0, css="data-key", width=_W_QTY, margin_end=_M_QTY)
        _r1c2 = _lbl("Sell",  xalign=1.0, css="data-key", width=_W_SELL)
        _r1c3 = _lbl("",      xalign=1.0, css="data-key", width=_W_AVG)
        self._hdr_grid.attach(_r1c0, 0, 1, 1, 1)
        self._hdr_grid.attach(_r1c1, 1, 1, 1, 1)
        self._hdr_grid.attach(_r1c2, 2, 1, 1, 1)
        self._hdr_grid.attach(_r1c3, 3, 1, 1, 1)

        # Row 2: separator
        sep = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
        self._hdr_grid.attach(sep, 0, 2, 4, 1)

        body.append(self._hdr_grid)

        # ── Data grid (inside ScrolledWindow, dynamic) ────────────────────────
        ...
        self._data_grid = Gtk.Grid()
        self._data_grid.set_column_spacing(4)
        self._data_grid.set_margin_start(4)
        self._data_grid.set_margin_end(12)
        self._data_grid.set_hexpand(True)
        scroll.set_child(self._data_grid)

        # Empty placeholder
        self._empty_lbl = _lbl("— empty —", xalign=0.5, css="data-key", hexpand=True)
        self._data_grid.attach(self._empty_lbl, 0, 0, 4, 1)
    ...
    def refresh(self) -> None:
        ...
        for item in enriched:
            ...
            if cat != current_cat:
                ...
                self._data_grid.attach(cat_lbl, 0, row, 1, 1)
                self._data_grid.attach(_lbl("", css="data-key", width=_W_QTY, margin_end=_M_QTY), 1, row, 1, 1)
                self._data_grid.attach(_lbl("", css="data-key", width=_W_SELL), 2, row, 1, 1)
                self._data_grid.attach(_lbl("", css="data-key", width=_W_AVG),  3, row, 1, 1)
            ...
            self._data_grid.attach(n_lbl, 0, row, 1, 1)
            self._data_grid.attach(_lbl(f"{count} t", xalign=1.0, css="data-key",
                                         width=_W_QTY, margin_end=_M_QTY), 1, row, 1, 1)
            self._data_grid.attach(_lbl(_fmt_cr(sell) if sell else "—",
                                         xalign=1.0, css="data-key", width=_W_SELL), 2, row, 1, 1)
            self._data_grid.attach(_lbl(_fmt_cr(avg) if avg else "—",
                                         xalign=1.0, css="data-key", width=_W_AVG),  3, row, 1, 1)
        ...
        self._data_grid.attach(sep2, 0, row, 4, 1)
        row += 1

        self._data_grid.attach(_lbl("Totals", css="data-key", hexpand=True),    0, row, 1, 1)
        self._data_grid.attach(_lbl(f"{used} t", xalign=1.0, css="data-key",
                                     width=_W_QTY, margin_end=_M_QTY),           1, row, 1, 1)
        self._data_grid.attach(_lbl(_fmt_cr(total_sell) if total_sell else "—",
                                     xalign=1.0, css="data-key", width=_W_SELL), 2, row, 1, 1)
        self._data_grid.attach(_lbl(_fmt_cr(total_avg)  if total_avg  else "—",
                                     xalign=1.0, css="data-key", width=_W_AVG),  3, row, 1, 1)
        self._totals_row = row

    def _clear_data_rows(self) -> None:
        for row_idx in set(self._item_grid_rows.values()) | set(self._cat_grid_rows.values()):
            for col in range(4):
                child = self._data_grid.get_child_at(col, row_idx)
                if child:
                    self._data_grid.remove(child)
        self._item_grid_rows.clear()
        self._cat_grid_rows.clear()
        if self._totals_row >= 0:
            for r in (self._totals_row - 1, self._totals_row):
                for col in range(4):
                    child = self._data_grid.get_child_at(col, r)
                    if child:
                        self._data_grid.remove(child)
            self._totals_row = -1
