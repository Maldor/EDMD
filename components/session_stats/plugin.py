"""
components/session_stats/plugin.py — Session timing and summary aggregator.
...
class SessionStatsPlugin(BasePlugin):
    ...
    def get_electron_data(self) -> dict:
        """Return a JSON-serialisable session snapshot for the Electron renderer."""
        from core.emit import fmt_duration
        rows: list[dict] = []
        for provider in getattr(self.core, "session_providers", []):
            try:
                provider_rows = provider.get_summary_rows()
                if provider_rows:
                    rows.append({
                        "provider": getattr(provider, "PLUGIN_DISPLAY",
                                            type(provider).__name__),
                        "rows": provider_rows,
                    })
            except Exception:
                pass
        dur = self.session_duration_seconds() if hasattr(self, "session_duration_seconds") else 0
        return {
            "duration":    fmt_duration(int(dur)) if dur else None,
            "providers":   rows,
        }
