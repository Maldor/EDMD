"""
components/cargo/plugin.py — Ship cargo inventory tracking.
...
class CargoPlugin(BasePlugin):
    ...
    def get_electron_data(self) -> dict:
        """Return a JSON-serialisable cargo snapshot for the Electron renderer.

        Price columns follow the same logic as the GTK4 block:
          col2 = tgt_price (Spansh target) if target set, else sell_price (docked)
          col3 = mean_price (galactic average) always
        When docked market data is absent, col2 falls back to mean_price.
        """
        s           = self.core.state
        items_raw    = getattr(s, "cargo_items",       {}) or {}
        market       = getattr(s, "cargo_market_info",  {}) or {}
        tgt_market   = getattr(s, "cargo_target_market", {}) or {}
        mean_prices  = getattr(s, "cargo_mean_prices",  {}) or {}
        commodities  = market.get("commodities", {})
        tgt_comms    = tgt_market.get("commodities", {})
        has_target   = bool(tgt_market)
        items = []
        for key, v in items_raw.items():
            count = v.get("count", 0)
            if count <= 0:
                continue
            mkt  = commodities.get(key, {})
            tgt  = tgt_comms.get(key, {})
            sell = mkt.get("sell_price") or 0
            # mean_price: prefer per-commodity market value, fall back to
            # the persistent cargo_mean_prices dict (populated by all
            # previous market visits, so covers mining goods not in current market)
            avg  = mkt.get("mean_price") or mean_prices.get(key, 0)
            tgt_p = tgt.get("sell_price") or 0
            # col2: spansh target price if set, else docked sell, else galactic avg
            col2 = tgt_p if (has_target and tgt_p) else (sell or avg)
            # col3: galactic average always
            col3 = avg
            items.append({
                "key":        key,
                "name":       v.get("name_local") or key.replace("_", " ").title(),
                "count":      count,
                "stolen":     v.get("stolen", False),
                "col2_price": col2,
                "col3_price": col3,
            })
        capacity = getattr(s, "cargo_capacity", 0) or 0
        used     = sum(i["count"] for i in items)
        return {
            "items":             sorted(items, key=lambda x: x["name"]),
            "capacity":          capacity,
            "used":              used,
            "has_target":        has_target,
            "market_name":       market.get("station_name", ""),
            "market_system":     market.get("star_system",  ""),
