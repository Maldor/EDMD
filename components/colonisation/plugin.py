"""
components/colonisation/plugin.py — Colonisation construction tracking with
...
_RAVEN_UA = f"EDMD/{VERSION} (ravencolonial integration)"
...
class ColonisationPlugin(BasePlugin):
    ...
    def on_load(self, core) -> None:
        ...
        self._api_thread: threading.Thread = threading.Thread(
            target=self._api_worker, daemon=True, name="edmd-colonisation-api"
        )
    ...
    def get_electron_data(self) -> dict:
        s     = self.core.state
        sites = getattr(s, "colonisation_sites", []) or []
        cargo = getattr(s, "cargo_items",         {}) or {}
        docked     = getattr(s, "colonisation_docked", False)
        cur_mid    = self._current_market_id
        out = []
        for site in sites:
            is_current = docked and site.get("market_id") == cur_mid
            site_cargo = cargo if is_current else {}
            resources  = []
            for key, rd in (site.get("resources") or {}).items():
                prov      = rd.get("provided", 0)
                req       = rd.get("required", 1)
                remaining = max(0, req - prov)
                cargo_entry = site_cargo.get(key, {})
                in_cargo = (cargo_entry.get("count", 0)
                            if isinstance(cargo_entry, dict)
                            else int(cargo_entry or 0))
                resources.append({
                    "name":      rd.get("name", key),
                    "required":  req,
                    "provided":  prov,
                    "remaining": remaining,
                    "pct":       round(prov / req * 100) if req else 100,
                    "in_cargo":  in_cargo,
                })
            resources.sort(key=lambda r: -r["remaining"])
            out.append({
                "market_id":    site.get("market_id"),
                "system":       site.get("system", ""),
                "station_name": site.get("station", ""),
                "progress_pct": round((site.get("progress", 0)) * 100),
                "complete":     site.get("complete", False),
                "failed":       site.get("failed",   False),
                "resources":    resources,
