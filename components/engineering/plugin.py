"""
components/engineering/plugin.py — Combined engineering materials inventory.
...
class EngineeringPlugin(BasePlugin):
    ...
    def get_electron_data(self) -> dict:
        """Return a JSON-serialisable engineering materials snapshot."""
        s = self.core.state

        def _to_list(d: dict) -> list:
            return sorted(
                [{"key": k, "name": v.get("name_local", k), "count": v.get("count", 0)}
                 for k, v in (d or {}).items()],
                key=lambda x: x["name"],
            )

        locker = getattr(s, "engineering_locker", {}) or {}
        return {
            "raw":          _to_list(getattr(s, "materials_raw",          {})),
            "manufactured": _to_list(getattr(s, "materials_manufactured", {})),
            "encoded":      _to_list(getattr(s, "materials_encoded",      {})),
            "locker": {
                "components":  _to_list(locker.get("components",  {})),
                "items":       _to_list(locker.get("items",       {})),
                "consumables": _to_list(locker.get("consumables", {})),
                "data":        _to_list(locker.get("data",        {})),
            },
        }
