            ...
            if ev == "Loadout":
                ...
                for mod in je.get("Modules", []):
                    ...
                    if "fighterbay" in item:
                        ...
                        if m:
                            slf_cap = max(
                                slf_cap, _FIGHTERBAY_CAPACITY.get(m.group(1), 1)
                            )
...
def bootstrap_slf(
    state: MonitorState, journal_dir: Path, trace_mode: bool = False
) -> None:
        ...
        try:
            ...
            for line in reversed(lines):
                    ...
                    if ev == "LaunchFighter" and not je.get("PlayerControlled", True):
                        ...
                        if not slf_type_known:
                            ...
                            if _ft:
                                ...
                                slf_type_known = True
                                trace(
                                    f"SLF bootstrap: type={state.slf_type!r} from LaunchFighter",
                                    trace_mode,
                                )
                ...
                if not slf_type_known and ev == "RestockVehicle":
                    ...
                    trace(
                        f"SLF bootstrap: type={state.slf_type!r} from RestockVehicle in {jpath.name}",
                        trace_mode,
                    )
                    slf_type_known = True
...
def bootstrap_crew(
                ...
                for line in reversed(lines):
                    ...
                    if ev == "CrewAssign":
                        ...
                        if name:
                            ...
                            trace(
                                f"Crew bootstrap: name from history: {name!r}",
                                trace_mode,
                            )
                            break
                        ...
                        if name:
                            ...
                            state.crew_active = True
                            trace(
                                f"Crew bootstrap: name from NpcCrewPaidWage: {name!r}",
                                trace_mode,
                            )
                            break
                    elif ev == "LoadGame":
            ...
            with open(jpath, mode="r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if ev == "CrewAssign" and je.get("Name") == state.crew_name:
                        t = (
                            datetime.fromisoformat(je["timestamp"])
                            if "timestamp" in je
                            else None
                        )
                        ...
                        ev == "NpcCrewRank" and je.get("NpcCrewName") == state.crew_name
                    ):
                        found_rank = je.get("RankCombat", found_rank)
                    elif (
                        ev == "NpcCrewPaidWage"
                        and je.get("NpcCrewName") == state.crew_name
                    ):
    ...
    if earliest_time:
        ...
        trace(
            f"Crew bootstrap: {state.crew_name} first seen at {earliest_time}",
            trace_mode,
        )
    ...
    for jpath in journals:
        ...
        for line in reversed(lines):
            ...
            if ev in RESET_EVENTS:
                ...
                trace(
                    f"hull bootstrap: {ev} found in {jpath.name}, hull=100", trace_mode
                )
            ...
            if ev == "Repair":
                ...
                if "hull" in item or item == "":
                    trace(
                        f"hull bootstrap: Repair found in {jpath.name}, hull=100",
                        trace_mode,
                    )
...
def bootstrap_burn_rate(
    ...
    candidates = [
        (dt, fuel)
        for dt, fuel, ship in reservoir_events
        if ship is None
        or current_ship is None
...
def bootstrap_missions(
    ...
    if not accepted:
        print(
            f"{Terminal.YELL}Mission bootstrap:{Terminal.END} No active massacre missions found"
        )
        return
...
def handle_event(
    ...
    if plugin_dispatch is not None:
        ...
        if not state.in_preload:
            ...
            if _qh_slot != getattr(state, "_last_summary_slot", -1):
                ...
                _sess_plugin = (
                    ...
                    if core and hasattr(core, "_plugins")
                    else None
                )
        ...
        if ev_name in ("Docked", "Location") and data_provider is not None:
            ...
            ev_name in ("Undocked", "SupercruiseEntry", "FSDJump")
    ...
    try:
        ...
        match j["event"]:
                ...
                if "$Pirate_OnStartScanCargo" in j["Message"]:
                    ...
                    if piratename not in active_session.recent_inbound_scans:
                        ...
                        count_str = (
                            ...
                            if settings.get("ExtendedStats")
                            else ""
                        )
                        pirate_str = (
                            f" [{piratename}]" if settings.get("PirateNames") else ""
                        )
            ...
            case "Bounty" | "FactionKillBond":
                ...
                if j["event"] == "Bounty":
                    ...
                    ship = normalise_ship_name(
                        j.get("Target_Localised") or j.get("Target")
                    )
                else:
                ...
                kills_t = (
                    f" x{active_session.kills}" if settings.get("ExtendedStats") else ""
                )
                kills_d = (
                    f"x{active_session.kills} " if settings.get("ExtendedStats") else ""
                )
                bv_str = (
                    ...
                    if settings.get("BountyValue")
                    else ""
                )
                victimfaction = j.get("VictimFaction_Localised") or j.get(
                    "VictimFaction", ""
                )
                ...
                emitter.emit(
                    ...
                    msg_discord=(
                        f"{kills_d}**{ship}{killtime_str}**{pirate_str}{bv_str}{bf_str}"
                    ),
                    emoji="💥",
                    sigil="*  KILL",
                    timestamp=logtime,
                    loglevel=notify["RewardEvent"],
                )
            ...
            case "ReservoirReplenished":
                ...
                if (
                    ...
                    fuel_remain = (
                        fmt_duration(j["FuelMain"] / fuel_hour * 3600)
                        if fuel_hour > 0
                        else None
                    )
                    if fuel_remain:
                ...
                if j["FuelMain"] < state.fuel_tank_size * FUEL_CRIT_THRESHOLD:
                    col = Terminal.BAD
                    fuel_loglevel = notify["FuelCritical"]
                    level = " critical!"
                elif j["FuelMain"] < state.fuel_tank_size * FUEL_WARN_THRESHOLD:
                    col = Terminal.WARN
                    fuel_loglevel = notify["FuelWarning"]
                    level = " low:"
            ...
            case "LoadGame":
                ...
                state.pilot_ship = normalise_ship_name(
                    j.get("Ship_Localised") or j.get("Ship")
                )
                if j.get("ShipName"):
            ...
            case "Interdiction" if j["Success"] == "true":  # specify conditions
                event_name = "Interdiction Complete"
                emoji = "🚨"
                sigil = ">  INFO"
                emitter.emit(
                    msg_term=f"{Terminal.WHITE}Interdiction completed{Terminal.END}",
                    msg_discord=f"Interdiction completed",
                    emoji="🚨",
                    sigil=">  INFO",
                    timestamp=logtime,
                    loglevel=notify["Interdiction"],
                    event="Interdiction",
                )
            ...
            case "Interdicted":
                ...
                sigil = "!  WARN"
                emitter.emit(
                    msg_term=f"{Terminal.BAD}You've been interdicted{Terminal.END}",
                    msg_discord=f"Interdiction completed",
                    emoji="🚨",
                    sigil="! WARN",
                    timestamp=logtime,
            ...
            case "ShipyardSwap":
                state.pilot_ship = normalise_ship_name(
                    j.get("ShipType_Localised") or j.get("ShipType")
                )
                emitter.emit(
        ...
        event_name = (
            j.get("event", LABEL_UNKNOWN) if isinstance(j, dict) else LABEL_UNKNOWN
        )
        logtime_fmt = (
...
def monitor_journal(
    ...
    with open(jfile, mode="r", encoding="utf-8") as file:
        ...
        if state.pp_power:
            ...
            _pp_merits = (
                f"  ({state.pp_merits_total:,} merits)" if state.pp_merits_total else ""
            )
        ...
        while True:
            ...
            if not line:
                ...
                if (
                    ...
                    recent_avg_secs = sum(active_session.recent_kill_times) / len(
